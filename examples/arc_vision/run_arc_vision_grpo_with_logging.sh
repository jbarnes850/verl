#!/bin/bash
# Arc Vision RL Training Script with Enhanced Logging
# This script captures all output with timestamps and proper log organization
...
# Create a symlink to latest logs
ln -sfn "$LOG_DIR" "$OUTPUT_DIR/latest_logs"

echo "========================================"
echo "Arc Vision Training Starting"
echo "Timestamp: $(date)"
...
# Launch VERL PPO training with enhanced logging
python3 -m verl.trainer.main_ppo \
    ...
    hydra.sweep.dir="$LOG_DIR/hydra/sweep" \
    hydra.job.chdir=False \
    $@ 2>&1 | tee >(add_timestamp >> "$MAIN_LOG") | tee >(grep -E "epoch|step:|loss:|reward:|entropy:|kl_" | add_timestamp >> "$METRICS_LOG") | add_timestamp
